...
pushd $SCRIPT_ORIGIN/../vagrantfiles/normal/$(cat $SCRIPT_ORIGIN/../config | grep ^MAINVM= | sed "s/.*=//")
  ...
  vagrant up
  if [ $IS_CONTAINER = True ] ; then 
    vagrant ssh -c "WORKSPACE=$VIRTUAL_WORKSPACE $other_params bash $VIRTUAL_WORKSPACE/in/$REPO_NAME/scripts/create_container_on_container_or_VM.sh $COUNTER $JDK True"
