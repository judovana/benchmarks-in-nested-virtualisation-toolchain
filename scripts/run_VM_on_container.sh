...
podman ps -all
podman run --rm --privileged --name running-cont-run-vm preparation-cont-jdk sh /test/scripts/run_on_VM.sh $COUNTER $JDK
...
podman ps -all
podman cp running-cont-run-vm:/results $WORKSPACE/../VM-in-container-results/${JDK_NAME}/${COUNTER}
#ls -l $RESULT_DIR

#podman rm running-cont-run-vm
